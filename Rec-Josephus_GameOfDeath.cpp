/*/////////////////////// Recursion:Aditya Verma: Josephus Problem or Game of Death /////////////////////
...
int killPersons(int start, int k, vector<int> persons)
{
    if (persons.size() == 1)
    {
        return persons[0];
    ...
    return killPersons(start, k, persons);
}

/////////////////////// Solution 2 (Iterative) ////////////////////
int killInLoop(vector<int> persons, int k)
{
    int start = 0;
...
int main()
{
    ...
    int start = 0;
    int lastPerson = killPersons(start, k, persons);
    cout << lastPerson << endl;
    cout << killInLoop(persons, k) << endl;
}
